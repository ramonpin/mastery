defmodule Mastery.Boundary.TemplateValidator do
  ...
  def validate_raw(raw) do
    check(String.match?(raw, ~r{\S}), {:error, "must be a string"})
  end
  ...
  def validate_generator(_generator), do: {:error, "must be a string to list or function/0 pair"}
